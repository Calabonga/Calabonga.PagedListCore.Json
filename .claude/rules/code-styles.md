...
- `using`-директивы — за пределами namespace (`csharp_using_directive_placement = outside_namespace` в `.editorconfig`).
...
### Неизменяемость
- `record` / `readonly record struct` — для DTO и результатов в тестах и в новых вспомогательных типах.
...
- Новые async-методы добавляй с параметром `CancellationToken cancellationToken = default` и пробрасывай его дальше.
